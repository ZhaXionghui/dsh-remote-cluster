...
*/
async function withEnvelope(tool, run, meta) {
	...
	try {
		...
		if (errorCode(error) === TOOL_ABORTED) throw error;
		const extra = typeof meta === "function" ? void 0 : meta;
		return {
			success: false,
...
function hostId(value) {
	if (value.trim().length === 0) throw new Error("host_id must be a non-empty string");
	...
	return () => {
		...
		return {
			...
			kind: [
				...
				"upload",
				"download"
			].some((word) => lower.includes(word)) ? "execute" : "read"
		};
	};
}
function throwIfAborted(result, exec) {
	if (result.aborted) throw new HarnessError(`remote host operation aborted (${exec.name})`, TOOL_ABORTED);
}
/** Read the transport currently carrying one host, if any. */
function transportOf(snapshot) {
	const { connection } = snapshot;
	if (connection.state === "connected-active" || connection.state === "connected-idle") return connection.transport;
}
...
function transferRequest(hostValue, localPath, remotePath, overwrite, timeoutMs, signal) {
	if (localPath.trim().length === 0) throw new Error("local_path must be a non-empty string");
	if (remotePath.trim().length === 0) throw new Error("remote_path must be a non-empty string");
	if (timeoutMs !== void 0 && (!Number.isFinite(timeoutMs) || timeoutMs <= 0)) throw new Error("timeout_ms must be a positive finite number");
	return {
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			id: {
				type: "string",
				required: true,
				description: "Stable non-empty host id."
			},
			label: {
				type: "string",
				required: true,
				description: "Display name shown in the Remote hosts panel."
			},
			kind: {
				type: "string",
				required: true,
				description: "Host classification: server or cluster."
			},
			hostname: {
				type: "string",
				required: true,
			...
			port: {
				type: "number",
				required: true,
				description: "SSH port from 1 to 65535."
			},
			user: {
				type: "string",
				required: true,
			...
			auth_mode: {
				type: "string",
				required: true,
				description: "Authentication kind: identity_file, agent_socket, or password."
			},
			auth_path: {
				type: "string",
				description: "Local path to the private-key file or SSH agent endpoint; omit for password mode."
			}
		},
		output: {
			schema: { type: "json" },
		...
		execute: async (args) => await withEnvelope("remote_host_create", async () => {
			...
			if (writer === void 0) throw new Error("remote_host_create is unavailable: the SSH remote-host provider or writable settings service is not mounted");
			if (args.kind !== "server" && args.kind !== "cluster") throw new Error("kind must be \"server\" or \"cluster\"");
			if (args.auth_mode !== "identity_file" && args.auth_mode !== "agent_socket" && args.auth_mode !== "password") throw new Error("auth_mode must be \"identity_file\", \"agent_socket\", or \"password\"");
			if (args.id.trim().length === 0 || args.label.trim().length === 0 || args.hostname.trim().length === 0 || args.user.trim().length === 0) throw new Error("id, label, hostname, and user must be non-empty strings");
			if (!Number.isInteger(args.port) || args.port < 1 || args.port > 65535) throw new Error("port must be an integer from 1 to 65535");
			if (args.auth_mode !== "password" && (args.auth_path === void 0 || args.auth_path.trim().length === 0)) throw new Error("auth_path must be a non-empty local path unless auth_mode is \"password\"");
			const host = {
				id: args.id,
				label: args.label,
				kind: args.kind,
				...
				port: args.port,
				user: args.user,
				...args.auth_mode === "identity_file" && args.auth_path !== void 0 ? { identityFile: args.auth_path } : {},
				...args.auth_mode === "agent_socket" && args.auth_path !== void 0 ? { agentSocket: args.auth_path } : {},
	...
	ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => await withEnvelope("remote_host_inspect", async () => {
			...
			return {
				facts: await ctx.remoteHosts.inspect(id, exec.signal),
				host: ctx.remoteHosts.status(id)
			};
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			retrust: {
				type: "boolean",
				description: "Set true only after the user explicitly confirms a newly presented server key."
			}
		},
		output: {
			schema: { type: "json" },
		...
		execute: async (args, exec) => await withEnvelope("remote_host_connect", async () => {
			const id = hostId(args.host_id);
			if (args.retrust === true) await ctx.remoteHosts.trustHostKey(id);
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			command: {
				type: "string",
				required: true,
				description: "POSIX shell command string sent through stdin."
			},
			timeout_ms: {
				type: "number",
				description: "Optional positive execution deadline in milliseconds; the provider clamps it to its configured cap."
			}
		},
		output: {
			schema: { type: "json" },
