...
let RemoteHostController = (() => {
	...
	return class RemoteHostController extends _classSuper {
		static {
			...
			__esDecorate(this, null, _openTerminal_decorators, {
				kind: "method",
				name: "openTerminal",
				static: false,
				private: false,
				access: {
					has: (obj) => "openTerminal" in obj,
					get: (obj) => obj.openTerminal
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			...
			__esDecorate(this, null, _authenticationStatus_decorators, {
				kind: "method",
				name: "authenticationStatus",
				static: false,
				private: false,
				access: {
					has: (obj) => "authenticationStatus" in obj,
					get: (obj) => obj.authenticationStatus
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _answerAuthentication_decorators, {
				kind: "method",
				name: "answerAuthentication",
				static: false,
				private: false,
				access: {
					has: (obj) => "answerAuthentication" in obj,
					get: (obj) => obj.answerAuthentication
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _cancelAuthentication_decorators, {
				kind: "method",
				name: "cancelAuthentication",
				static: false,
				private: false,
				access: {
					has: (obj) => "cancelAuthentication" in obj,
					get: (obj) => obj.cancelAuthentication
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		async inspect(hostId, signal) {
			...
			try {
				return {
					facts: await this.ctx.remoteHosts.inspect(id, signal),
					host: this.ctx.remoteHosts.status(id)
				};
			} catch (error) {
				signal.throwIfAborted();
				if (error instanceof RemoteHostError && error.code === "HOST_NOT_FOUND") throw new RemoteError("remote-host/not-found", `no remote host "${hostId}" is configured`, { hostId });
		...
		*/
		async delete(hostId) {
			...
			try {
				...
				if (error instanceof RemoteHostError && error.code === "HOST_NOT_FOUND") throw new RemoteError("remote-host/not-found", `no remote host "${hostId}" is configured`, { hostId });
				if (error instanceof RemoteHostError && error.code === "REMOVE_UNSUPPORTED") throw new RemoteError("remote-host/remove-unsupported", `remote host "${hostId}" cannot be removed by its provider`, { hostId }, { cause: error });
		...
		*/
		async connect(hostId, signal) {
			...
			try {
				...
				signal.throwIfAborted();
				if (error instanceof RemoteHostError && error.code === "HOST_NOT_FOUND") throw new RemoteError("remote-host/not-found", `no remote host "${hostId}" is configured`, { hostId });
				if (error instanceof RemoteHostError && error.code === "AUTH_INTERACTIVE_REQUIRED") throw new RemoteError("remote-host/interactive-login-required", error.message, { hostId }, { cause: error });
				throw new RemoteError("remote-host/unavailable", `remote host "${hostId}" is unavailable`, { hostId }, { cause: error });
			}
		}
		/**
...
//#endregion
export { RemoteHostController, RemoteHostController as default };
