...
const name = "remote-host-invariant";
...
function assertTransport(hostId, transport, fail) {
	if (!TRANSPORTS.includes(transport)) fail(`remote host "${hostId}" connected state does not name an owning transport`);
}
/**
...
const install = Object.assign((ctx, fail) => {
	for (const host of ctx.remoteHosts.list()) {
		...
		if (connection.state === "connected-active") {
			if (!Number.isSafeInteger(connection.activeLeases) || connection.activeLeases < 1) fail(`remote host "${hostId}" connected-active state requires a positive lease count`);
			assertTransport(hostId, connection.transport, fail);
		}
		if (connection.state === "connected-idle") {
			if (!(connection.reclaimAt > connection.connectedAt)) fail(`remote host "${hostId}" idle reclaimAt must follow connectedAt`);
			assertTransport(hostId, connection.transport, fail);
		}
	}
}, { inject: ["remoteHosts"] });
