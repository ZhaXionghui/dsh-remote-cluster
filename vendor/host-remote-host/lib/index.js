...
var RemoteHostError = class extends Error {
	code;
	/**
...
*/
function RemoteHostAuthId(value) {
	return brandString(value);
}
const MAX_AUTH_CONNECTION_DURATION_MS = 2147483647;
/** Effect-scoped registry and dispatch service for configured remote hosts. */
var RemoteHostRegistry = class extends Service {
	...
	*/
	register(backend) {
		...
		return this.ctx.effect(() => {
			this.assertActive();
			if (id.length === 0) throw new Error("remote host id must be non-empty");
			if (backend.summary.label.trim().length === 0) throw new Error(`remote host "${id}" label must be non-empty`);
			if (backend.summary.hostname.trim().length === 0) throw new Error(`remote host "${id}" hostname must be non-empty`);
			if (this.backends.has(id)) throw new RemoteHostError(`remote host "${id}" is already registered`, "DUPLICATE_HOST");
	...
	*/
	async openTerminal(id) {
		this.assertActive();
		const backend = this.expect(id);
		if (backend.openTerminal === void 0) throw new RemoteHostError(`remote host "${id}" has no native terminal launcher`, "TERMINAL_UNAVAILABLE");
	...
	*/
	async remove(id) {
		this.assertActive();
		const backend = this.expect(id);
		if (backend.remove === void 0) throw new RemoteHostError(`remote host "${id}" cannot be removed by its provider`, "REMOVE_UNSUPPORTED");
	...
	*/
	beginAuthentication(id, request) {
		...
		if (duration !== void 0 && (!Number.isFinite(duration) || duration <= 0 || duration > MAX_AUTH_CONNECTION_DURATION_MS)) throw new RemoteHostError(`authentication connection duration must be a positive finite number no greater than ${MAX_AUTH_CONNECTION_DURATION_MS}`, "AUTH_INVALID_RESPONSE");
		for (const session of this.authSessions.values()) if (session.hostId === id && (session.snapshot.state === "pending" || session.snapshot.state === "prompt")) throw new RemoteHostError(`remote host "${id}" already has an authentication attempt`, "AUTH_SESSION_BUSY");
	...
	*/
	authenticationStatus(id) {
		...
		if (session === void 0) throw new RemoteHostError(`authentication session "${id}" was not found`, "AUTH_SESSION_NOT_FOUND");
		return session.snapshot;
	}
	/**
	...
	*/
	answerAuthentication(id, responses) {
		const session = this.authSessions.get(id);
		if (session === void 0) throw new RemoteHostError(`authentication session "${id}" was not found`, "AUTH_SESSION_NOT_FOUND");
		if (session.snapshot.state !== "prompt" || session.answer === void 0) throw new RemoteHostError(`authentication session "${id}" is not waiting for input`, "AUTH_INVALID_RESPONSE");
		const expected = session.snapshot.prompt.prompts.length;
		if (responses.length !== expected) throw new RemoteHostError(`authentication session "${id}" expected ${expected} response(s)`, "AUTH_INVALID_RESPONSE");
		const answer = session.answer;
		session.answer = void 0;
		session.snapshot = {
			id: session.id,
	...
	*/
	cancelAuthentication(id) {
		...
		session.answer?.reject(/* @__PURE__ */ new Error("authentication cancelled"));
		session.answer = void 0;
		session.controller.abort(/* @__PURE__ */ new Error("authentication cancelled"));
		session.snapshot = {
			id: session.id,
			hostId: session.hostId,
			state: "cancelled"
	...
	*/
	async trustHostKey(id) {
		this.assertActive();
		const backend = this.expect(id);
		if (backend.trustHostKey === void 0) throw new RemoteHostError(`remote host "${id}" does not support host-key trust`, "HOST_KEY_UNTRUSTED");
	...
	*/
	...
	async dispatchTransfer(direction, backend, request) {
		if (backend.transfer === void 0) throw new RemoteHostError(`remote host "${backend.summary.id}" does not support file transfer`, "TRANSFER_UNSUPPORTED");
		return await backend.transfer(direction, request);
	}
	assertActive() {
		if (this.disposing) throw new RemoteHostError("remote host service is disposing", "SERVICE_DISPOSING");
	}
	expect(id) {
		const backend = this.backends.get(id);
		if (backend === void 0) throw new RemoteHostError(`remote host "${id}" is not registered`, "HOST_NOT_FOUND");
		return backend;
	}
	async disposeAll() {
		this.disposing = true;
		for (const authId of this.authSessions.keys()) this.cancelAuthentication(authId);
		this.authSessions.clear();
		const backends = [...this.backends.values()];
		this.backends.clear();
		const failures = (await Promise.allSettled(backends.map((backend) => backend.dispose()))).filter((result) => result.status === "rejected").map((result) => result.reason);
		if (failures.length > 0) throw new AggregateError(failures, "failed to dispose remote host backends");
	}
};
//#endregion
export { RemoteHostAuthId, RemoteHostError, RemoteHostId, RemoteHostRegistry, RemoteHostRegistry as default };
